# Genba

> **2026-08-04:** 外資SaaS営業インテリジェンスメディアへピボット。新事業の正本は [事業計画](docs/09-pivot-business-plan.md)、サイト実装の正本は [Webリニューアル指示書](docs/10-website-renewal-spec.md)。日本語メディアの初期版を実装済み。

**Genba — 外資戦士と予備軍の作戦会議室。**

外資SaaS企業の日本採用、営業求人、組織の変化を公式情報から追う無料インテリジェンスメディア。

このリポジトリは、事業構想・ブランド定義・コンテンツ制作・競合調査などGenba立ち上げに関する情報を一元管理する場所。

## AI運用

このプロジェクトはCodexとClaude Codeを併用する。両者は同じ [`PROJECT_RULES.md`](PROJECT_RULES.md) を自動読込する構成とし、片側だけに事業ルールを追加してはならない。

- Codex入口: `AGENTS.md` → `PROJECT_RULES.md`
- Claude Code入口: `CLAUDE.md` → `PROJECT_RULES.md`
- 共有日次フロー: [`ops/daily-workflow.md`](ops/daily-workflow.md)
- 同期検査: `npm run validate:agents`
- 公開前検査: `npm run check`

## ディレクトリ構成

```
docs/            事業の中核ドキュメント(ブランド、事業概要、サービス設計など)
content/         発信コンテンツ(ブログ・X投稿)の下書き・公開済み・カレンダー・テンプレート
research/        競合・市場データなどの一次情報ストック
ops/             ガードレール、法務・利益相反チェックなど運用上の管理事項
```

## docs/ の目次

| ファイル | 内容 |
|---|---|
| [01-brand.md](docs/01-brand.md) | ブランド名・タグライン・命名の由来 |
| [02-business-overview.md](docs/02-business-overview.md) | 事業概要・提供価値 |
| [03-founder-profile.md](docs/03-founder-profile.md) | 創業者(Jio)の経歴・強み |
| [04-target-customers.md](docs/04-target-customers.md) | ターゲット顧客・セグメント |
| [05-service-design.md](docs/05-service-design.md) | サービス内容・料金モデル |
| [06-content-strategy.md](docs/06-content-strategy.md) | コンテンツ・発信戦略 |
| [07-competitive-landscape.md](docs/07-competitive-landscape.md) | 競合状況・差別化ポイント |
| [08-open-items.md](docs/08-open-items.md) | 未確定・要検討事項 |
| [09-pivot-business-plan.md](docs/09-pivot-business-plan.md) | 外資SaaS営業インテリジェンスメディアへのピボット後の事業計画 |
| [10-website-renewal-spec.md](docs/10-website-renewal-spec.md) | ピボットに対応するWebサイト・データ・自動運用の実装指示書 |

## ローンチまでのアクションリスト

Jio自身がメディア運用・X運用を開始できる状態に持っていくための実行タスクは [ops/launch-checklist.md](ops/launch-checklist.md) にまとめている。

求人追加、職業紹介との境界、候補者情報の扱い、スポンサー商品と価格については、[求人掲載・スポンサー運用ポリシー](ops/job-publication-and-sponsor-policy.md) を運用上の正本とする。

## 現在のステータス (2026-08-04時点)

- ピボット後の事業計画・サイト仕様: 完了
- 日本語メディア初期版: 実装済み
- 企業・求人・採用シグナルの初期データ: 公式情報をもとに公開
- 法人マネタイズ: 就業規則・利益相反の確認後に開始
