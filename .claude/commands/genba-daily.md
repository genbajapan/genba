...
- Then write the Japanese version by re-composing the same ideas in Japanese, not by translating the English sentence-by-sentence. A literal rendering reads as translated English — long clause-chained sentences, calque phrasing, English-style connectors — even when every word is technically correct Japanese. Think in Japanese from the argument outward: natural business-Japanese sentence rhythm (shorter sentences, native causal/contrastive connectors like ため/一方で/むしろ, idiomatic phrasing), matching the tone of the existing files in `content/blog/published-ja/`. This version's real readers are Japanese sales/business people (per the About page's founding story) — write for them, don't just make the English legible to them.
