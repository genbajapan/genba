...
`デイリータスク`で作成した号は、Kit入稿が完成次第、Jioの確認用アドレス`js@genbajapan.com`へ直ちにテスト送信する。Kit上の送信成功、デスクトップ・スマートフォンpreview、各CTAの実遷移、送信者情報、配信停止導線、配信対象数、timezoneを確認し、すべて合格して実行時点が19:30前なら、Jioの返信・確認・承認を待たずに同日19:30（Asia/Tokyo）へ予約する。Jio本人によるメールの開封・返信・受信報告は予約条件にしない。Kit上の送信成功またはその他の必須項目を確認できない場合は、予約・本配信を行わず理由を報告する。
...
- 読了時間は3分を上限とする。各社は2つの短い段落、20〜28秒程度で伝える。第1段落は`何を解決する会社か → その会社固有の面白さ → 今回公式確認した求人・採用・日本進出の動き`、第2段落は太字の`仕事として見ると：`に`役割が向き合う課題 → 活かせる具体的な経験・領域 → 通常の営業・製品説明・進行管理を超えて任される責任 → 積める経験・強み`を記す。人数、担当社数、同時案件数、待機対応、評価KPI、給与など、多くの会社に共通する未確認項目を定型列挙して締めない。企業・役割固有で応募判断に重要な未確認点だけを必要に応じて残す。英語は会社名・製品名・公式求人名・必要な定着略語に絞り、一般語は導入文、領域ラベル、本文、CTAを通じて自然な日本語へ直す。日本未進出企業は後段を`日本進出を先回りすると：`へ置き換え、日本求人・法人・拠点などの重要な未確認点を残す
...
- 週3回を超える臨時配信は原則行わない
- 公式情報とGenba分析を分ける
- `デイリータスク`で作成した号の完成直後のJio宛テスト送信と、毎号テスト合格後の同日19:30予約は、2026-08-24および2026-08-27のJioによる継続承認済みの必須工程として、対話実行・スケジュール実行のどちらでも追加確認なく実行する。それ以外の臨時配信、即時配信、別時刻への変更、配信対象の変更は、その回の明示承認がある場合だけ実行する
...
**仕事として見ると：** [役割が向き合う課題]。[活かせる具体的な業界・職種・業務経験]。[製品説明や通常の進行管理を超えて任される責任]。[その結果として積める経験・次につながる強み]。
...
- 各カードは大きな画像や企業ロゴを使わず、薄いグレーの区切り線、十分な余白、会社名、領域、状態ラベル、`会社の説明・固有の面白さ・今回の動き`、太字の`仕事として見ると：`と`活かせる経験・任される責任・積める強み`、CTAの固定順で構成する。日本未進出企業は後段を`日本進出を先回りすると：`とし、日本求人・法人・拠点などの重要な未確認点を残す
